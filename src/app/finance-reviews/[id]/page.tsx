...
function Field({ label, value }: { label: string; value: unknown }) { return <div><strong>{label}</strong><p>{displayText(value)}</p></div>; }
...
function FinanceReviewDetail({ id, uid }: { id: string; uid: string }) {
  ...
  async function load() { setError(null); try { const snap = await getDoc(doc(getClientDb(), "finance_reviews", id)); setReview(snap.exists() ? ({ id: snap.id, ...snap.data() } as FinanceReview) : null); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : "Failed to load finance review."); } finally { setLoaded(true); } }
