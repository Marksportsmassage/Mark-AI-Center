...
function CapitalAllocationDetail({ id, uid }: { id: string; uid: string }) {
  ...
  async function load() { setError(null); try { const snap = await getDoc(doc(getClientDb(), "capital_allocations", id)); setAllocation(snap.exists() ? ({ id: snap.id, ...snap.data() } as CapitalAllocation) : null); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : "Failed to load capital allocation."); } finally { setLoaded(true); } }
