...
export interface LineStore {
  getLineEvent(id: string): Promise<Record<string, unknown> | null>;
  getLineUser(hash: string): Promise<LineUserRecord | null>;
  upsertLineUser(hash: string, data: Partial<LineUserRecord>): Promise<void>;
  createLineEvent(data: Record<string, unknown>, id?: string): Promise<string>;
  updateLineEvent(id: string, data: Record<string, unknown>): Promise<void>;
  createInbox(data: Record<string, unknown>): Promise<string>;
  updateInbox(id: string, data: Record<string, unknown>): Promise<void>;
  createTaskDispatch(data: Record<string, unknown>): Promise<string>;
  createRouteLog(data: Record<string, unknown>): Promise<string>;
  createAuditLog(data: Record<string, unknown>): Promise<string>;
}

export interface LineReplyClient {
  reply(input: { replyToken: string; accessToken: string; message: string }): Promise<void>;
}

export interface LineWebhookEnv {
  lineChannelSecret?: string;
  lineChannelAccessToken?: string;
  lineReplyEnabled?: boolean;
  webhookMode?: "disabled" | "capture_only" | "active_private";
}
...
export interface LineWebhookLogSummary {
  error_code: string | null;
  error_message_redacted: string | null;
  event_shape_summary: string;
  has_events: boolean;
  event_count: number;
  has_user_id: boolean;
...
function eventShapeSummary(body: unknown): LineWebhookLogSummary {
  const parsedBody = body && typeof body === "object" ? (body as { events?: unknown }) : {};
  ...
  const source = firstEvent.source && typeof firstEvent.source === "object" ? (firstEvent.source as Record<string, unknown>) : {};
  const message = firstEvent.message && typeof firstEvent.message === "object" ? (firstEvent.message as Record<string, unknown>) : {};
...
function response(status: number, body: Record<string, unknown>, log: LineWebhookLogSummary) {
  return { status, body, log };
}

function lineReplyMode(enabled?: boolean) {
  return enabled === true ? "enabled_ack_only" : "disabled";
}

function processedAckMessage(needsClarification: boolean) {
...
async function writeReplyAudit(
  store: LineStore,
  input: { action: string; lineEventId: string; reason: string; now: string; replyStatus: LineReplyStatus }
) {
...
export function verifyLineSignature(rawBody: Buffer, signature: string | undefined, channelSecret: string | undefined) {
...
function redactLineEvent(event: Record<string, unknown>, userId?: string) {
    ...
    source: {
      type:
        event.source && typeof event.source === "object"
          ? (event.source as Record<string, unknown>).type
          : undefined,
...
function mockLineRoute(rawText: string) {
  const lower = rawText.toLowerCase();
  const includes = (items: string[]) => items.some((item) => lower.includes(item.toLowerCase()));
  ...
  if (
    includes(["服飾", "選品", "測試資金", "初期資金", "初期投入", "批發", "進貨", "庫存", "毛利", "回本"]) &&
...
export async function processLineWebhook(request: LineWebhookRequest) {
  ...
  try {
    body = JSON.parse(request.rawBody.toString("utf8")) as { events?: Array<Record<string, unknown>> };
  } catch {
    return response(
      200,
      { ok: true, processed: 0, ignored: true, reason: "invalid_json", reply_sent: false },
      {
    ...
    try {
      ...
      if (route.task_needed && !route.needs_clarification) {
        taskId = await request.store.createTaskDispatch({
          ...
          instructions:
            route.task_dispatch?.instructions?.join("\n") ??
